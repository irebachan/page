...
// ----------------------------------------------------------------------------
// Version
// 1.0.2 2020/06/06 一部の処理を共通部分に移行
// 1.0.1 2020/03/29 動的イベントを消去するとエラーになる問題を修正
// 1.0.0 2020/03/15 MV版から流用作成
// ----------------------------------------------------------------------------
...
(() => {
    ...
    Game_Interpreter.prototype.getEventIdForEventReSpawn = function(idOrName, isTemplate) {
        ...
        if (!isNaN(idOrName)) {
            ...
            const event    = DataManager.searchDataItem(dataList, 'name', idOrName);
            id             = event ? event.id : 0;
        }
    ...
    Game_Map.prototype.isErsCheckTerrainTag = function(type, x, y) {
        return type.some(function(id) {
            if (x === 9 && y === 10) {
                console.log(this.terrainTag(x, y), id);
            }
